...
Projeto acadêmico de um site de assinatura para o EvoPG, desenvolvido na disciplina de Laboratório de Engenharia de Software.
...
O foco atual do trabalho está em **planejamento, modelagem, prototipação e estruturação arquitetural**. Ou seja: o projeto **ainda não está pronto** e **não representa uma aplicação finalizada**.

## Estado atual

Neste momento, o repositório contém principalmente:

- páginas e protótipos iniciais de interface;
- scripts front-end de apoio ao fluxo proposto;
- funções e migrações que servem como base técnica da solução;
- documentação acadêmica em LaTeX;
- diagramas UML em PlantUML.

Parte da implementação já foi esboçada para apoiar a documentação e validar decisões de arquitetura, mas o sistema ainda está em evolução.
...
- Este repositório deve ser entendido como **material de projeto**, não como produto concluído.
- A arquitetura, os fluxos e os diagramas podem mudar conforme a evolução da disciplina.
- Alguns trechos de código representam **protótipos técnicos** e **decisões em validação**.

## Próximos passos

- refinar os requisitos funcionais e não funcionais;
- consolidar os diagramas e a documentação;
- amadurecer o fluxo de assinatura e primeiro acesso;
- evoluir a modelagem de dados e integrações;
- transformar o protótipo em uma implementação mais estável.
